"""
Structural sensitivity analysis: fixed starting point.

For each layer, every cell is treated in turn as the forced start node.
The script re-solves the exact open-path model and compares the result with the
base model.

Outputs:
- fixed_starting_point_details.csv
- fixed_starting_point_summary.csv
- fixed_starting_point_plot.png
"""
...
DETAIL_CSV = SCRIPT_DIR / "fixed_starting_point_details.csv"
SUMMARY_CSV = SCRIPT_DIR / "fixed_starting_point_summary.csv"
PLOT_FILE = SCRIPT_DIR / "fixed_starting_point_plot.png"
...
def main() -> None:
    ...
    for schedule_item in base_results["schedule"]:
        ...
        for cell in cells:
            candidate_path, solver_status = solve_open_path_with_constraints(
                ...
                fixed_start_cell_id=cell.cell_id,
            )
        ...
        summary_rows.append(
            [
                ...
                layer_id,
                layer_label,
                base_layer["start_cell_id"],
                len(cells),
                f"{base_total_time_s:.6f}",
                best_candidate["fixed_start_cell_id"],
                f"{best_candidate['layer_total_time_s']:.6f}",
                f"{best_candidate['delta_time_percent']:.6f}",
                worst_candidate["fixed_start_cell_id"],
    ...
    axes[0].set_title("Fixed-Start Sensitivity by Layer")
    ...
    axes[1].set_title("Fixed-Start Response Summary")
