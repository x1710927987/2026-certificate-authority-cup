...
def main() -> None:
    ...
    for ax, scheme_name in zip(axes, SCHEME_ORDER):
        ...
        values_by_cell = {row["cell_id"]: to_float(row["phi"]) for row in scheme_rows}
    ...
    fig.suptitle(
        f"Fig 2-5 Neighborhood nonuniformity index map on the representative layer\n{representative_layer_title(representative)}",
        fontsize=14,
        y=0.98,
    )
    cbar = fig.colorbar(colorbars[-1], ax=axes, shrink=0.85, location="right")
    cbar.set_label("phi")
    output_path = OUTPUT_DIR / "fig2_5_phi_map.png"
    fig.savefig(output_path, dpi=220, bbox_inches="tight")
    plt.close(fig)

    save_metadata(
        OUTPUT_DIR / "fig2_5_phi_map.json",
        {
            "figure_id": "fig2_5",
