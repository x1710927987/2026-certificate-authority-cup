...
def main() -> None:
    ...
    ax.set_title("Fig 2-15 Sensitivity to self-heating strength A0")
    ax.set_xlabel("A0")
    ax.set_ylabel("weighted total thermal risk")
    ax.grid(alpha=0.3)
    ax.legend(frameon=False)
    output_path = OUTPUT_DIR / "fig2_15_A0_sensitivity.png"
    fig.tight_layout()
    fig.savefig(output_path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    save_metadata(OUTPUT_DIR / "fig2_15_A0_sensitivity.json", {"figure_id": "fig2_15", "input_csv": str(INPUT_CSV), "output_image": str(output_path)})
