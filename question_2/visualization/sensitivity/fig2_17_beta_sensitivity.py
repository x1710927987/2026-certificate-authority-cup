...
def main() -> None:
    ...
    ax.set_title("Fig 2-17 Sensitivity to spatial decay coefficient beta")
    ax.set_xlabel("beta")
    ax.set_ylabel("weighted total thermal risk")
    ax.grid(alpha=0.3)
    ax.legend(frameon=False)
    output_path = OUTPUT_DIR / "fig2_17_beta_sensitivity.png"
    fig.tight_layout()
    fig.savefig(output_path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    save_metadata(OUTPUT_DIR / "fig2_17_beta_sensitivity.json", {"figure_id": "fig2_17", "input_csv": str(INPUT_CSV), "output_image": str(output_path)})
